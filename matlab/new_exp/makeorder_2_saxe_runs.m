...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Write CSV to be used to make par compatible w/ mrVista
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for r = 1:nruns
    ...
    csvfilename = fullfile(participant_folder, strcat('saxe_parfile_for_run', num2str(run_number), '.csv'));
    fid = fopen(csvfilename, 'w');
    if fid == -1
        error('Failed to open file: %s', csvfilename);
    end
    ...
    for i = 1:ntrials 
        ...
        % Update onset time based on trial
        if mod(i,6) == 0
            onset_time = onset_time + (viddur - 0.0001);
        elseif mod(i,3) == 0
            current_block = current_block + 1;
            onset_time = onset_time + (viddur - 0.0001);
        else
